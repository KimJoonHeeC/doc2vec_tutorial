...
class Seq2Seq:
    ...
    def cell(self, n_hidden, output_keep_prob):
        _rnn_cell = tf.contrib.rnn.BasicRNNCell(self.n_hidden)
        rnn_cell = tf.contrib.rnn.DropoutWrapper(_rnn_cell, output_keep_prob=output_keep_prob)
        return rnn_cell
    ...
    def train(self, session, enc_input, dec_input, targets):
        return session.run([self.train_op, self.cost], feed_dict={self.enc_input: enc_input,
    ...
    def test(self, session, enc_input, dec_input, targets):
        ...
        return session.run([self.targets, self.outputs, accuracy], feed_dict={self.enc_input: enc_input,
                                                                              self.dec_input: dec_input,
                                                                              self.targets: targets})

    def predict(self, session, enc_input, dec_input):
        return session.run(self.outputs, feed_dict={self.enc_input: enc_input,
                                                    self.dec_input: dec_input})
